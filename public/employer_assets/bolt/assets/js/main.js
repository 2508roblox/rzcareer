...
var krajeeDialogDefaults_19466c26 = {"alert": {"type": "type-info", "title": "Thông tin", "buttonLabel": "<span class=\"glyphicon glyphicon-ok\"></span> Đồng ý"}, "confirm": {"type": "type-warning", "title": colorgbConfirmTitle, "btnOKClass": "btn-warning", "btnOKLabel": "<span class=\"glyphicon glyphicon-ok\"></span> "+colorgbConfirmOk, "btnCancelLabel": "<span class=\"glyphicon glyphicon-ban-circle\"></span> "+colorgbConfirmCancel}, "prompt": {"draggable": false, "title": "Thông tin", "buttons": [{"label": "Hủy bỏ", "icon": "glyphicon glyphicon-ban-circle"}, {"label": "Đồng ý", "icon": "glyphicon glyphicon-ok", "class": "btn-primary"}], "closable": false}, "dialog": {"draggable": true, "title": "Thông tin", "buttons": [{"label": "Hủy bỏ", "icon": "glyphicon glyphicon-ban-circle"}, {"label": "Đồng ý", "icon": "glyphicon glyphicon-ok", "class": "btn-primary"}]}};
...
function boltJs() {
    ...
    $('#navbar-second-toggle > ul > li > a, .dropdown-menu-right > li > a').each(function () {
        var pn = pathname.toLowerCase().replace('/index', '');
        if (pn.split('/').pop() === $(this).attr('href').split('/').pop()) {
            $(this).parent().addClass('active');
        }
    ...
    $('.company-profile .editable').editable({
        url: baseUrl + '/ajax/company-profile',
        type: 'textarea',
        pk: 1,
...
$(function () {
    $(document).on('submit', '.search-pjax', function (e) {
...
$(document).ready(function(){
    $('.search-pjax select,.search-pjax input,.search-pjax textarea').on('change', function() {
        $('.search-pjax').submit();
    });
})
...
// Listen from iframe message
window.addEventListener("message", function(event) {
    if (event.data.event_id === "iframeClosed") {
        event.data.data.current_url = window.location.href;

        sendTrackingVisibleTime(event.data.data);
    }
});

function sendTrackingVisibleTime(visibleData) {
    $.ajax({
        url: '/ajax/update-time-visibility-cv',
        type: 'GET',
        data: visibleData,
    }).success(function (response) {
